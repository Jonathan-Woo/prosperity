...
class Trader:
    ...
    def basket(self):
        # try:
            ...
            if ((mid_berry > 0) & (mid_choc > 0) & (mid_roses > 0) & (mid_basket > 0)):
                ...
                self.traderData.accept_product_price('diff_historical', diff)

                # if len(self.traderData.get_product_price('diff_historical')) > 4000:
                #     self.traderData.drop_product_price('diff_historical')

                diff_historical = self.traderData.get_product_price('diff_historical')
                ...
                if self.state.timestamp > 50000:
                    ...
                    # deviance_historical = self.basket_params['deviance_historical']
                    # deviance = abs(diff - avg)/std
                    # deviance_historical = pd.concat([deviance_historical, pd.Series(deviance)], ignore_index=True)
                    # self.basket_params['deviance_historical'] = deviance_historical
                    #
                    # deviance_historical = deviance_historical.sort_values()
                    # index = list(deviance_historical).index(deviance) + 1
                    # deviance_percentile = index / len(deviance_historical)
                    ...
                    if diff > avg + std:
                        ...
                        for ask in asks_basket:
                            # If we have inventory space
                            if position_limit > 0:
                                if taken < size_per_take//2:
                                    order_quantity = min(position_limit, size_per_take//2)
                                    position_limit -= order_quantity
                                    taken += order_quantity
                                    orders_basket.append(Order('GIFT_BASKET', ask[0] - 1, -order_quantity))
                            else:
                                break
                        ...
                        for bid in bids_basket:
                            ...
                            if position_limit > 0:
                                if taken < size_per_take//2:
                                    ...
                                    position_limit -= order_quantity
                                    taken += order_quantity
                                    orders_basket.append(Order('GIFT_BASKET', bid[0] + 1, order_quantity))
                            else:
                                break
    ...
    def underlying(self, product, person):
        self.result[product] = []

        best_bid = None
        best_ask = None

        if product in self.state.order_depths:
            bids = sorted(list(self.state.order_depths[product].buy_orders.items()), key=lambda x: x[0], reverse=True)
            asks = sorted(list(self.state.order_depths[product].sell_orders.items()), key=lambda x: x[0], reverse=False)
            best_bid = bids[0][0]
            best_ask = asks[0][0]
        ...
        market_trades = self.state.market_trades.get(product, [])
