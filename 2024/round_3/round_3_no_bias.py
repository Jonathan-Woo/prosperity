...
class Trader:
    ...
    def __init__(self):
        ...
        self.rose_params = {
            "position_limit": 60,
            "LR_coefs": [
                -0.009756025357141743,
                0.0015927597562190126,
                0.01512771483360309,
                0.9930342246276632,
            ],
        }
        ...
        self.chocolate_params = {
            ...
            "LR_coefs": [
                -0.0069364303816369,
                0.004755708324684231,
                0.018611912031832507,
                0.983567709920092,
            ],
        }
    ...
    def linear_predict_price(self, lr_coefs, product):
        ...
        """
        assert len(lr_coefs) == len(self.traderData.get_product_price(product))
        expected_price = np.array(lr_coefs).dot(
            np.array(self.traderData.get_product_price(product))
        )
    ...
    def etf(self):
        def market_make(product, product_params):
            ...
            window_size = len(product_params["LR_coefs"])
            self.result[product] = []
