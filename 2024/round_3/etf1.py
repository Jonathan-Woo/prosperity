...
class Trader:
    ...
    def etf(self):
        pos_limit = {'CHOCOLATE': 250, 'STRAWBERRIES': 350,'ROSES': 60, 'GIFT_BASKET': 60}
        curr_pos = {'CHOCOLATE':  self.state.position.get('CHOCOLATE', 0), 
        ...
        if "CHOCOLATE" in self.state.order_depths:
            bids_choc = sorted(list(self.state.order_depths["CHOCOLATE"].buy_orders.items()), key=lambda x: x[0], reverse=True)
            asks_choc = sorted(list(self.state.order_depths["CHOCOLATE"].sell_orders.items()), key=lambda x: x[0], reverse=False)
        ...
        if 'STRAWBERRIES' in self.state.order_depths:
            bids_berry = sorted(list(self.state.order_depths["STRAWBERRIES"].buy_orders.items()), key=lambda x: x[0], reverse=True)
            asks_berry = sorted(list(self.state.order_depths["STRAWBERRIES"].sell_orders.items()), key=lambda x: x[0], reverse=False)
        ...
        if 'ROSES' in self.state.order_depths:
            bids_roses = sorted(list(self.state.order_depths["ROSES"].buy_orders.items()), key=lambda x: x[0], reverse=True)
            asks_roses = sorted(list(self.state.order_depths["ROSES"].sell_orders.items()), key=lambda x: x[0], reverse=False)
        ...
        if 'GIFT_BASKET' in self.state.order_depths:
            bids_etf = sorted(list(self.state.order_depths["GIFT_BASKET"].buy_orders.items()), key=lambda x: x[0], reverse=True)
            asks_etf = sorted(list(self.state.order_depths["GIFT_BASKET"].sell_orders.items()), key=lambda x: x[0], reverse=False)
        ...
        if mid_etf - nav > 380 + margin_of_safety:
            ...
            "MARKET ORDERS"
            etf_qty = -min(bids_etf[0][1], pos_limit['GIFT_BASKET']+curr_pos['GIFT_BASKET'])
            assert etf_qty < 0
        ...
        if mid_etf - nav < 380 - margin_of_safety: 
            ...
            """MARKET ORDERS"""
            etf_qty = min(-asks_etf[0][1], pos_limit['GIFT_BASKET']-curr_pos['GIFT_BASKET'])
            curr_pos['GIFT_BASKET'] += etf_qty
    ...
    def chocolate(self):
        if "CHOCOLATE" in self.state.position:
            cur_position = self.state.position["CHOCOLATE"]
        else:
            cur_position = 0
        # Position limits dictate that current position + aggregated buys must not exceed position limit
        # and current position - aggregated sells must not exceed position limit
        # So, keep track of the new long and short positions
        new_long_position, new_short_position = cur_position, cur_position
        position_limit = self.CHOCOLATE_params["position_limit"]
        window_size = len(self.CHOCOLATE_params["LR_coefs"])
        self.result["CHOCOLATE"] = []

        highest_market_bid = max(self.state.order_depths["CHOCOLATE"].buy_orders.keys())
        lowest_market_ask = min(self.state.order_depths["CHOCOLATE"].sell_orders.keys())
        mid_price = (highest_market_bid + lowest_market_ask) / 2

        # Update the trader data with the latest price
        self.traderData.accept_product_price("CHOCOLATE", mid_price)
        if len(self.traderData.get_product_price("CHOCOLATE")) > window_size:
            self.traderData.drop_product_price("CHOCOLATE")
            assert len(self.traderData.get_product_price("CHOCOLATE")) == window_size
        ...
        # Compute expected price
        expected_price = np.array(self.CHOCOLATE_params["LR_coefs"]).dot(
            self.traderData.get_product_price("CHOCOLATE")
        )
        expected_price = round(expected_price)
        ...
        # Buy undervalued
        # Buy at value if we are short
        for ask, qty in self.state.order_depths["CHOCOLATE"].sell_orders.items():
            if (
                (ask < expected_price) or ((cur_position < 0) and (ask == mid_price))
            ) and new_long_position < position_limit:
                order_qty = min(-qty, position_limit - new_long_position)
                new_long_position += order_qty
                self.result["CHOCOLATE"].append(Order("CHOCOLATE", ask, order_qty))

        # Sell overvalued
        # Sell at value if we are long
        for bid, qty in self.state.order_depths["CHOCOLATE"].buy_orders.items():
            if (
                (bid > expected_price) or ((cur_position > 0) and (bid == mid_price))
            ) and new_short_position > -position_limit:
                order_qty = min(qty, position_limit + new_short_position)
                new_short_position -= order_qty
                self.result["CHOCOLATE"].append(Order("CHOCOLATE", bid, -order_qty))
        ...
        if new_long_position < position_limit:
            ...
            new_long_position += order_qty
            order_price = min(tighter_bid, expected_price - 1)
            self.result["CHOCOLATE"].append(Order("CHOCOLATE", order_price, order_qty))
        if new_short_position > -position_limit:
            order_qty = position_limit + new_short_position
            new_short_position -= order_qty
            order_price = max(tighter_ask, expected_price + 1)
            self.result["CHOCOLATE"].append(Order("STARFRUIT", order_price, -order_qty))
