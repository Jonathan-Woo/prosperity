...
class Trader:
    ...
    def etf(self):
        ...
        if mid_etf - nav < 380 - margin_of_safety:
            ...
            """MARKET MAKING ATTEMPT 1"""
            # if etf_market_trades is not None:
            #     prices = [trade.price for trade in etf_market_trades]
            #     prices = [i+1 for i in prices if best_bid_etf <= i + 1 <= nav + 380 - margin_of_safety]
            #     if len(prices)!=0:

            #         div = round(pos_limit['GIFT_BASKET']-curr_pos['GIFT_BASKET'])//len(prices)
            #         for price in prices:
            #             orders_etf.append(Order('GIFT_BASKET', price, div))

            """MARKET MAKING ATTEMPT 2"""

            # if best_bid_etf + 1 <= nav + 380 - margin_of_safety:
            #     orders_etf.append(Order("GIFT_BASKET", best_bid_etf+1, round(etf_qty)))
            ...
            #         l3 = bids_etf[2][0]
            #     if l1-l2 > 1:
            #         step = (pos_limit['GIFT_BASKET']-curr_pos['GIFT_BASKET'])//(l1-l2)
            #         for i in range(1, l1-l2+1):
            #             orders_etf.append(Order("GIFT_BASKET", l1-i, step))
            #     elif l1-l2<=1 and l3 is not None:
            #         if l2-l3>1:
            #             step = (pos_limit['GIFT_BASKET']-curr_pos['GIFT_BASKET'])//(l2-l3)
            #             for i in range(1, l2-l3+1):
            #                 orders_etf.append(Order("GIFT_BASKET", l2-i, step))
