...
class Trader:
    ...
    def etf(self):
        ...
        if mid_etf - nav > 380 + margin_of_safety:
            ...
            for bid in bids_etf:
                # If bid price facilitates arbitrage
                if bid[0] > 380 + nav + margin_of_safety:
                    ...
                    if position_limit > 0:
                        if taken < size_per_take:
                            ...
                            orders_etf.append(Order('ORCHIDS', bid[0], -order_quantity))
                    else:
                        break
                else:
                    break
            ...
            # place asks when short signal
            undercut_ask_price = best_ask_etf - 1
            max_iterations = (position_limit//size_per_price)+1
            for i in range(1, max_iterations+1):
                if position_limit > 0:
                    if undercut_ask_price > 380 + nav + margin_of_safety:
                        order_for = -min(size_per_price, position_limit)
                        position_limit += order_for
                        orders_etf.append(Order('GIFT_BASKET', undercut_ask_price+i, order_for))
                        curr_pos['GIFT_BASKET'] += order_for
                        undercut_ask_price = undercut_ask_price - 1
                else:
                    break
        ...
        if mid_etf - nav < 380 - margin_of_safety:
            ...
            for ask in asks_etf:
                # If bid price facilitates arbitrage
                if ask[0] < 380 + nav - margin_of_safety:
                    # If we have inventory space
                    if position_limit > 0:
                        if taken < size_per_take:
                            ...
                            position_limit -= order_quantity
                            taken += order_quantity
                            orders_etf.append(Order('ORCHIDS', ask[0], order_quantity))
                    else:
                        break
                else:
                    break
            ...
            # """LIMIT ORDERS"""
            # place bids when long signal
            undercut_bid_price = best_bid_etf + 1
            max_iterations = (position_limit//size_per_price)+1
            for i in range(1, max_iterations+1):
                if position_limit > 0:
                    if undercut_bid_price < 380 + nav - margin_of_safety:
                        order_for = min(size_per_price, position_limit)
                        position_limit += order_for
                        orders_etf.append(Order('GIFT_BASKET', undercut_bid_price-i, order_for))
                        curr_pos['GIFT_BASKET'] += order_for
                        undercut_bid_price = undercut_bid_price + 1
                else: break
